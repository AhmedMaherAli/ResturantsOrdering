    ...
    public class OrdersController
    {
        ...
        public Order CreateOrder(MakeOrderViewModel orderViewModel)
        {
            ...
            foreach( string line in orderViewModel.OrderItems.Split("\r\n"))
            {
                ...
                bool canParseQuantity = int.TryParse(operands[0], out Quantity);
                if(canParseItemID && canParseQuantity && operands.Length == 2)
                {
                    Item item = menuController.GetItemFromMenuById(Convert.ToInt32(operands[0]));
                    if (item == null)
                        return null;
                    item.Quantity = Convert.ToInt32(operands[1]);
                    orderItems.Add(item);
                }
                else
                {
